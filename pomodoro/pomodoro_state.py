...
class WorkingState(PomodoroState):
    ...
    def __init__(self, pomodoro):
        ...
        self.max_time = int(self.pomodoro.configuration['max_work_time'])
        self.elapsed_time = 0
...
class RestingState(PomodoroState):
    def __init__(self, pomodoro):
        ...
        self.max_time = int(self.pomodoro.configuration['max_rest_time'])

    def next_state(self):
        if self.pomodoro.configuration['continue_working_after_rest']:
            self.pomodoro.state = self.pomodoro.working_state
        else:
            self.pomodoro.state = self.pomodoro.waiting_state
...
class PomodoroMachine(object):
    ...
    def working_percentage(self):
        return int(100 * self.state.elapsed_time / int(self.configuration['max_work_time']))

    def is_waiting(self):
        return self.state.current_state() == WAITING_STATE

    def is_working(self):
        return self.state.current_state() == WORKING_STATE
